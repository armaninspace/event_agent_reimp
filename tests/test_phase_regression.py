...
def test_run_phase_regression_writes_summary_and_checks_artifacts(tmp_path: Path) -> None:
    ...
    assert summary.prior_knowledge_evolved_duplicate_candidate_count == 0
    assert summary.selected_unique_semantic_slot_count == 3
    assert summary.selected_semantic_slot_counts == {
        "city_week_event_spending": 1,
